...
#if defined(CONFIG_MSM_REMOTE_SPINLOCK_LDREX) || \
		defined(CONFIG_MSM_REMOTE_SPINLOCK_AUTO)
...
static int dt_node_is_valid(const char *compat)
{
	struct device_node *node;
	const char *status;
	int statlen;
...
{
#ifdef CONFIG_MSM_REMOTE_SPINLOCK_DEKKERS
	/* Use Dekker's algorithm when LDREX/STREX and SWP are unavailable */
	pr_info("using dekker's algorithm\n");
	...
	/* Use SWP-based locks when LDREX/STREX are unavailable */
	pr_info("using swp\n");
	...
	is_hw_lock_type = 0;
#elif defined(CONFIG_MSM_REMOTE_SPINLOCK_LDREX)
	/* Use LDREX/STREX for shared memory locking, when available */
	pr_info("using ldrex/strex\n");
	...
	pr_info("using sfpb hardware mutex registers\n");
	...
	current_ops.owner = __raw_remote_gen_spin_owner;
	is_hw_lock_type = 1;
#elif defined(CONFIG_MSM_REMOTE_SPINLOCK_AUTO)
	/* Use DT info to configure; fallback to LDREX if DT is missing */
	if (dt_node_is_valid(sfpb_compatible_string)) {
		pr_info("using sfpb hardware mutex registers (auto)\n");
		current_ops.lock = __raw_remote_sfpb_spin_lock;
		current_ops.unlock = __raw_remote_sfpb_spin_unlock;
		current_ops.trylock = __raw_remote_sfpb_spin_trylock;
		current_ops.release = __raw_remote_gen_spin_release;
		current_ops.owner = __raw_remote_gen_spin_owner;
		is_hw_lock_type = 1;
	} else if (dt_node_is_valid(ldrex_compatible_string)) {
		pr_info("using ldrex/strex (auto)\n");
		current_ops.lock = __raw_remote_ex_spin_lock;
		current_ops.unlock = __raw_remote_ex_spin_unlock;
		current_ops.trylock = __raw_remote_ex_spin_trylock;
		current_ops.release = __raw_remote_gen_spin_release;
		...
		pr_warn("using ldrex/strex (fallback)\n");
		current_ops.lock = __raw_remote_ex_spin_lock;
		current_ops.unlock = __raw_remote_ex_spin_unlock;
		current_ops.trylock = __raw_remote_ex_spin_trylock;
		current_ops.release = __raw_remote_gen_spin_release;
...
#else
#error "Spinlock implementation undefined"
#endif
}
