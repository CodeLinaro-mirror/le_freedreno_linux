...
static inline void pm_wake_lock(const char *buf)
{ }
static inline void pm_wake_unlock(const char *buf)
{ }

static inline void modem_queue_start_reset_notify(void)
{ }

static inline void modem_queue_smsm_init_notify(void)
{ }
...
static void do_smd_probe(void)
{
	...
	struct smem_area smem;

	if (smem_init(&smem))
		return;
	shared = (void *)smem.virt_addr;
...
static int smsm_init(struct smem_area *smem)
{
	struct smem_shared *shared;
	int i;
...
int smd_core_platform_init(struct platform_device *pdev, struct smem_area *smem)
{
	int i;
	int ret;
	uint32_t num_ss;
...
static int msm_smd_probe(struct platform_device *pdev)
{
	...
	if (pdev) {
		if (pdev->dev.of_node) {
			ret = smd_core_devicetree_init(pdev, &smem);
			if (ret) {
			...
			ret = smd_core_platform_init(pdev, &smem);
			if (ret) {
				pr_err(
	...
	pr_info("SMD successfully initialized\n");
