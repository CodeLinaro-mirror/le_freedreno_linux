...
// hmm, always propagating -EBUSY up and doing the wait after
// dropping all our locks, as is done for -EDEADLK, might be
// a good idea.  In theory we cannot deadlock here, because
// drop_locks() doesn't happen until all locks are grabbed,
// so there is no chance for a lock we hold to block the one
// who has this lock.  But maybe it is more consistent to
// handle this in the same way as -EDEADLK.
