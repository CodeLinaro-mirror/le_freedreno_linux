...
struct pm8xxx_of_data {
	struct pm8xxx_regulator_core_platform_data core_data;
	struct pm8xxx_vreg vreg;
	struct pm8xxx_regulator_platform_data pdata;
};

static int pm8xx_of_vreg_data(struct device *dev, struct pm8xxx_vreg *vreg)
{
	struct device_node *np = dev->of_node;
	u32 val;
	int ret;

	mutex_init(&vreg->pc_lock);

	if (of_device_is_compatible(np, "qcom,pm8xxx-regulator-vs300")) {
		vreg->type = PM8XXX_REGULATOR_TYPE_VS300;

		ret = of_property_read_string(np, "qcom,rdesc-name",
				&vreg->rdesc.name);
		if (ret)
			return ret;

		ret = of_property_read_u32(np, "qcom,vreg-ctrl-addr", &val);
		if (ret)
			return ret;
		vreg->ctrl_addr = val;

		ret = of_property_read_u32(np, "qcom,vreg-test-addr", &val);
		if (ret)
			return ret;
		vreg->test_addr = val;
	...
	/* TODO add other regulator types.. so far just adding what I need.. */

	return -EINVAL;
}

// XXX hack, get this from DT..
static struct regulator_consumer_supply vreg_consumers_HDMI_MVS[] = {
	REGULATOR_SUPPLY("8921_hdmi_mvs",	NULL),
	REGULATOR_SUPPLY("hdmi_mvs",		"hdmi_msm.0"),
};

static int pm8xx_of_pdata_data(struct device *dev,
		struct pm8xxx_regulator_platform_data *pdata)
{
	struct device_node *np = dev->of_node;
	u32 val;
	...
	if (of_device_is_compatible(np, "qcom,pm8xxx-regulator-vs300")) {
		...
		// XXX check dev->id.. not sure if that is correct..
		pdata->id = dev->id;
		pdata->init_data.constraints.valid_ops_mask = REGULATOR_CHANGE_STATUS;

		ret = of_property_read_string(np, "qcom,rdesc-name",
				&pdata->init_data.constraints.name);
		if (ret)
			return ret;

		ret = of_property_read_u32(np, "qcom,pdata-always-on", &val);
		if (ret)
			return ret;
		pdata->init_data.constraints.always_on = val;

		ret = of_property_read_u32(np, "qcom,pdata-pull-down", &val);
		if (ret)
			return ret;
		pdata->pull_down_enable = val;

		ret = of_property_read_u32(np, "qcom,pdata-enable-time", &val);
		if (ret)
			return ret;
		pdata->enable_time = val;

		ret = of_property_read_string(np, "qcom,pdata-supply-regulator",
				&pdata->init_data.supply_regulator);
		if (ret)
			return ret;

		// XXX hack, get this from DT..
		pdata->init_data.num_consumer_supplies = ARRAY_SIZE(vreg_consumers_HDMI_MVS);
		pdata->init_data.consumer_supplies = vreg_consumers_HDMI_MVS;
...
static struct pm8xxx_regulator_core_platform_data *
pm8xxx_of_data(struct device *dev)
{
	struct device_node *np = dev->of_node;
	struct pm8xxx_of_data *of_data;
	struct pm8xxx_regulator_core_platform_data *core_data;
	struct pm8xxx_vreg *vreg;
	struct pm8xxx_regulator_platform_data *pdata;
	u32 val;
	...
	of_data = kzalloc(sizeof(*of_data), GFP_KERNEL);
	if (!of_data) {
		ret = -ENOMEM;
		goto fail;
	}

	core_data = &of_data->core_data;
	vreg = &of_data->vreg;
	pdata = &of_data->pdata;

	core_data->vreg = vreg;
	core_data->pdata = pdata;

	ret = of_property_read_u32(np, "qcom,is-pin-controlled", &val);
	if (ret)
		goto fail;
	core_data->is_pin_controlled = val;

	ret = pm8xx_of_vreg_data(dev, vreg);
	if (ret)
		goto fail;

	ret = pm8xx_of_pdata_data(dev, pdata);
	if (ret)
		goto fail;
...
static int pm8xxx_vreg_probe(struct platform_device *pdev)
{
	...
	if (np) {
		core_data = pm8xxx_of_data(&pdev->dev);
		if (IS_ERR(core_data))
	...
	cfg.dev = &pdev->dev;
	cfg.init_data = &(pdata->init_data);
	cfg.driver_data = vreg;
	cfg.of_node = pdev->dev.of_node;

	if (!core_data->is_pin_controlled) {
		vreg->rdev = regulator_register(rdesc, &cfg);
		if (IS_ERR(vreg->rdev)) {
		...
		vreg->rdev_pc = regulator_register(rdesc, &cfg);
...
static const struct of_device_id dt_match[] = {
	{ .compatible = "qcom,pm8xxx-regulator" },
	{}
};
